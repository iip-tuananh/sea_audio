...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['categories'] = Category::query()->where('show_home_page',1)->with('image')->orderBy('sort_order')->get();
        $data['imageBlocks'] = Amenities::query()->with('image')->orderBy('order_number')->get();
        $data['categoriesSpecial'] = CategorySpecial::query()->with(['products' => function ($query) {
            $query->with(['image', 'image_back', 'category'])
                ->where('status', 1);
        }])->orderBy('order_number')->get();

        $categories = Category::query()->where('show_home_page',1)->orderBy('sort_order')->get();
        $categories->each(function($cat) {
            $dataProd = $cat->products()
                ->with(['image','category'])
                ->where('status',1)
                ->orderBy('created_at','desc')
                ->limit(6)
                ->get();
            $cat->setRelation('products', $dataProd);
        });
        $data['categoriesProduct'] = $categories;
        $data['posts'] = Post::query()
            ->where('status', 1)
            ->with('image')->latest()->get()->take(3);
